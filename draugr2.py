...
NIST_DB = "C:\\Users\\P09816\\OneDrive - NGC\\Desktop\\Coding\\cve_scanner\\04.Resources\\nist_800_53.json"
...
class FileRow(QWidget):
    ...
    def __init__(self, label_text, placeholder="", is_save=False, file_filter="", pick_folder=False, parent=None):
    ...
    def _browse(self):
        if self.pick_folder:
            ...
            if self.is_save:
                path, _ = QFileDialog.getSaveFileName(self, "Save Report As...", "", self.file_filter)
            else:
...
def _format_threat_map(rows: List[Dict[str, Any]], limit: int = 0) -> str:
    ...
    tier1.append(
        "| CVE ID | Sev | Risk | Ver | KEV | Public Exploit | EPSS | CWEs |"
    )
    tier1.append("|--------|-----|------|-----|-----|----------------|------|------|")
...
.report-header img { height: 52px; width: auto; flex-shrink: 0; }
...
def build_executive_report_markdown(
    ...
    report_title: str = "Draugr Threat Intelligence Executive Report",
) -> str:
    # ---- 1️⃣ Gather **only version‑confirmed** rows for the whole report ----
    exec_rows = [
        r for r in all_rows
        if str(r.get("Version Confirmed", "")).strip().upper() == "YES"
    ]

    # ---- 2️⃣ Build the per‑software body (unchanged apart from variable name) ----
    grouped: Dict[Tuple[str, str], List[Dict[str, Any]]] = defaultdict(list)
    for row in exec_rows:
        key = (
            str(row.get("Software Name", "")).strip(),
            str(row.get("Software Version", "")).strip(),
        )
        grouped[key].append(row)

    lines: List[str] = []
    lines.append(
        "This report summarises the highest‑priority, version‑confirmed threats per software item. "
        "For the full findings see the Comprehensive Report."
    )
    lines.append("")

    for (software_name, software_version), all_sw_rows in sorted(
        grouped.items(), key=lambda x: x[0][0].lower()
    ):
        # rows are already version‑confirmed, just sort by risk
        all_sw_rows.sort(key=lambda r: _to_float(r.get("Risk Score", 0)), reverse=True)
        rows = all_sw_rows                     # keep the name “rows” for the
                                               # existing _format_* calls
        if not rows:
            continue

        # ----- the original block that builds the software‑specific markdown -----
        total_cves = len(rows)
    ...
    # ---- 3️⃣ Insert the new narrative sections ---------------------------------
    exec_md = _executive_summary(exec_rows)      # top of report
    conc_md = _conclusion_section(exec_rows)     # bottom of report
    full_md = f"{exec_md}\n{body_md}\n{conc_md}"
...
def build_comprehensive_report_markdown(
    all_rows: List[Dict[str, Any]],
    report_title: str = "Draugr Threat Intelligence Comprehensive Report",
) -> str:
    # ---- the comprehensive report works with **all** rows (no filtering) ----
    grouped: Dict[Tuple[str, str], List[Dict[str, Any]]] = defaultdict(list)
    for row in all_rows:
        key = (
            str(row.get("Software Name", "")).strip(),
    ...
    lines: List[str] = []
    lines.append(
        "This report contains the complete findings for every scanned software item, "
        "including all CVEs, threat mappings, scenarios, and mitigations."
    )
    lines.append("")

    for (software_name, software_version), rows in sorted(
        grouped.items(), key=lambda x: x[0][0].lower()
    ):
        ...
        total_cves = len(rows)
        confirmed = sum(
            1 for r in rows if str(r.get("Version Confirmed", "")).strip().upper() == "YES"
        )
        critical = sum(1 for r in rows if str(r.get("CVSS Severity", "")).upper() == "CRITICAL")
        high = sum(1 for r in rows if str(r.get("CVSS Severity", "")).upper() == "HIGH")
        medium = sum(1 for r in rows if str(r.get("CVSS Severity", "")).upper() == "MEDIUM")
        ...
        lines.append(f"- Version Confirmed CVEs: {confirmed}")
        lines.append(f"- Critical: {critical} | High: {high} | Medium: {medium} | Low: {low}")
        lines.append(f"- Known Exploited: {kev_count}")
        lines.append(f"- Public Exploits Available: {exploit_count}")
        lines.append(f"- Overall Risk Rating: {overall_risk} ({max_risk})")
    ...
    # ---- 3️⃣ Use the same narrative helpers but on *all* rows ---------------
    exec_md = _executive_summary(all_rows)   # top of the report
    conc_md = _conclusion_section(all_rows)  # bottom of the report
    full_md = f"{exec_md}\n{body_md}\n{conc_md}"
